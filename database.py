    ...
    try:
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS Votes (
                ...
                mobile_number TEXT,  -- Added mobile_number column
        ...
        if 'mobile_number' not in columns:
            try:
                cursor.execute('ALTER TABLE Votes ADD COLUMN mobile_number TEXT')
                print("Added mobile_number column to Votes table")
...
def record_vote(user_id, room_id, candidate_id, mobile_number=None):
    """Record a user's vote with mobile number."""
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO Votes (voter_id, room_id, candidate_id, mobile_number)
            VALUES (?, ?, ?, ?)
        ''', (user_id, room_id, candidate_id, mobile_number))
