...
# Optional OCR engines with fallback
try:
...
# Initialize EasyOCR reader lazily (cached globally to avoid slow re-init)
_ocr_reader = None
...
# win32com is only loaded conditionally on Windows
# Not using resource_filename, so we can remove this import
import base64
import numpy as np
import re
...
# Define reCAPTCHA keys from environment variables
RECAPTCHA_SITE_KEY = os.getenv("RECAPTCHA_SITE_KEY", "6Ldr9BwrAAAAAH7lQbGirPpJO3kiAmyZR2nxMNWF")
RECAPTCHA_SECRET_KEY = os.getenv("RECAPTCHA_SECRET_KEY", "6Ldr9BwrAAAAAIoAWQE-wa0T5fL0tcP8xSjHqcL-")

# OTP Configuration
OTP_EXPIRY_MINUTES = int(os.getenv("OTP_EXPIRY_MINUTES", "5"))

app = Flask(__name__)
app.secret_key = os.getenv('FLASK_SECRET_KEY', 'frvs-stable-secret-key-2024')

# Add this after app initialization
app.config.update(
    RECAPTCHA_SITE_KEY=RECAPTCHA_SITE_KEY,
    RECAPTCHA_SECRET_KEY=RECAPTCHA_SECRET_KEY,
    WTF_CSRF_CHECK_DEFAULT=False  # Disable global CSRF check; protect specific forms as needed
)

# Initialize CSRF protection
csrf = CSRFProtect(app)

# Return JSON for CSRF errors (instead of HTML) so AJAX callers can handle them
from flask_wtf.csrf import CSRFError
@app.errorhandler(CSRFError)
def handle_csrf_error(e):
    return jsonify({'success': False, 'message': f'CSRF error: {e.description}'}), 400
...
DATABASE_PATH = os.getenv(
    'DATABASE_PATH',
    os.path.join(os.path.dirname(os.path.abspath(__file__)), 'frvs.db')
)

# Ensure essential runtime directories exist
for _dir_name in ['uploads', 'votes', 'data', 'models']:
    os.makedirs(os.path.join(os.path.dirname(os.path.abspath(__file__)), _dir_name), exist_ok=True)

# Initialize database schema automatically on application load
try:
    from database import init_db
    init_db()
except Exception as _db_init_err:
    print(f"Warning: Database auto-init: {_db_init_err}")
...
def speak(text):
    """Text-to-speech functionality using Windows SAPI if available."""
    try:
...
def vote(room_id):
    ...
    try:
        ...
        # Render vote.html directly without redirecting to check_liveness
...
def cast_vote(room_id):
    ...
    try:
        ...
        mobile_number = session.get('voter_mobile', '')  # Get mobile from session
        ...
        try:
            ...
            # Prepare vote data with mobile number
            ...
            vote_data = [
                ...
                mobile_number,      # Mobile Number (added)
            ...
            cursor.execute('''
                INSERT INTO Votes (room_id, candidate_id, voter_id, mobile_number)
                VALUES (?, ?, ?, ?)
            ''', (room_id, candidate_id, user_id, mobile_number))

            # Write to CSV with updated headers
            votes_dir = os.path.join(app.root_path, 'votes')
            ...
            with open(csv_file, 'a', newline='') as f:
                ...
                if not file_exists:
                    writer.writerow([
                        ...
                        'Voter Name',
                        'Voter Aadhar',
                        'Mobile Number',  # Added mobile number column
                        'Candidate ID',
                        'Candidate Name'
                    ])
            ...
            session.pop('voter_mobile', None)  # Clear mobile from session
...
@csrf.exempt  # Room credentials (host_id + room_id) serve as the authentication
def join_room():
    if request.method == 'POST':
        conn = None
        cursor = None
    ...
    try:
        ...
        if reader is not None:
            ...
            try:
                ...
                print(f"Pytesseract error: {_tess_err}")
...
# Note: CSRFProtect already initialized above after app config
...
def check_session():
    if request.endpoint and request.endpoint != 'static':
        ...
        excluded_routes = [
            ...
            'process_id_card', 'cast_vote', 'verify_face',
            'verify_voter_details', 'send_otp', 'verify_otp',  # Voter AJAX routes
            'votecasted', 'flash_message', 'video_feed',
            'join_room', 'validate_room', 'results'  # Room login and public results
        ]
...
def results(room_id):
    try:
        ...
        election = cursor.fetchone()
        if not election:
            flash(f'Voting room {room_id} not found', 'error')
            return redirect(url_for('index'))
        ...
        # Query votes including mobile_number
        cursor.execute('''
            SELECT 
                v.timestamp,
                u.name as voter_name,
                u.user_id as voter_id,
                v.mobile_number,
        ...
        for v in votes:
            cid = v['candidate_id']
            candidate_votes_count[cid] = candidate_votes_count.get(cid, 0) + 1

        candidates = []
        for row in candidates_raw:
            cid = row['candidate_id']
            votes_count = candidate_votes_count.get(cid, 0)
            percentage = round((votes_count / total_votes * 100), 1) if total_votes > 0 else 0.0
            
            logo_base64 = None
            if row['logo_data']:
                try:
                    logo_base64 = base64.b64encode(row['logo_data']).decode('utf-8')
                except Exception as logo_err:
                    print(f"Error encoding logo for candidate {cid}: {logo_err}")
                    logo_base64 = None
...
def send_otp():
    try:
        if not twilio_client or not TWILIO_VERIFY_SERVICE_SID:
            return jsonify({
                'success': False,
                'message': 'SMS OTP service is not configured on this server.'
            }), 503
        ...
        formatted_number = f'+91{mobile}'  # Adding India country code

        try:
            print(f"Sending OTP to: {formatted_number}")
            print(f"Using Verify Service SID: {TWILIO_VERIFY_SERVICE_SID}")
            ...
            if verification.status == 'pending':
                # Store the mobile number in session for verification
                session['verification_mobile'] = formatted_number
                return jsonify({
                    'success': True,
                ...
                return jsonify({
                    'success': False,
                    'message': f'Failed to send OTP: {verification.status}'
                }), 400
...
def verify_otp():
    try:
        ...
        # Get stored mobile number from session
        mobile = session.get('verification_mobile')
        ...
        try:
            ...
            if verification_check.status == 'approved':
                # Clear the verification mobile from session
                session.pop('verification_mobile', None)
                
                # Store mobile number in session for later use in votes.csv
                session['voter_mobile'] = mobile.replace('+91', '')  # Store without country code
...
if __name__ == '__main__':
    try:
        ...
        # Preload EasyOCR reader if installed
